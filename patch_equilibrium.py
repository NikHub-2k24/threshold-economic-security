...
with open("core/equilibrium.py", "r") as f:
    content = f.read()

# We want to replace the b_behavioral_star = min(...) logic with tracking.
# We will just write a new get_destabilization_thresholds function.

new_func = """def get_destabilization_thresholds(game: Game, baseline_profile: Tuple[Strategy, ...], split_rule: str = "attacker_optimal") -> Dict[str, Any]:
...
content = re.sub(r'def get_destabilization_thresholds\(.*', new_func, content, flags=re.DOTALL)

with open("core/equilibrium.py", "w") as f:
    f.write(content)
